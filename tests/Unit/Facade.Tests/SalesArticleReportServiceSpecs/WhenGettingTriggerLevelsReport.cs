namespace Linn.Products.Facade.Tests.SalesArticleReportServiceSpecs
{
    ...
    public class WhenGettingTriggerLevelsReport : ContextBase
    {
        ...
        public void SetUp()
        {
            this.SalesArticleReports.SalesArticleTriggerLevelsReport()
                .Returns(new ResultsModel { ReportTitle = new NameModel("title") });
            this.result = this.Sut.GetSalesArticleTriggerLevels();
        }

        [Test]
        public void ShouldGetResults()
        {
            this.SalesArticleReports.Received().SalesArticleTriggerLevelsReport();
        }
