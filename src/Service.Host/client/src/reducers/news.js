    ...
    switch (action.type) {
        ...
        case RECEIVE_NEWS:
            return {
                ...state,
                loading: false,
                unseen: action.payload.news.notifications.filter(
                    n => !localStorage.getItem(n.title)
                ),
                seen: action.payload.news.notifications.filter(n => localStorage.getItem(n.title))
            };
